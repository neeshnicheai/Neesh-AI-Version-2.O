...
class InsightService {
    ...
    async getRisks(projectId) {
        ...
        return questions.map(q => {
            ...
            if (q.ask_count > 2)
                action = "High demand! Add Manual Answer immediately.";
            else if (ageDays > 3)
                action = "Stale question. Archive or Answer.";
            else
                action = "Monitor or Supplement docs.";
            return {
    ...
    async getReadiness(projectId) {
        ...
        // NOT_READY Logic
        if (unansweredPct > 50)
            reasons.push("More than 50% of questions are unanswered.");
        if (lowPct > 25)
            reasons.push("High risk: >25% of answers have LOW confidence.");
        if (health.avgResolutionTimeHours > 72)
            reasons.push("Slow learning velocity (> 3 days to resolve).");
        if (reasons.length > 0) {
            return {
                status: 'NOT_READY',
                reason: reasons.join(" "),
