...
class LearningService {
    ...
    async logQuestion(projectId, questionText) {
    ...
    async logAnswer(questionId, answer, confidence, isAi) {
        const { data, error } = await this.supabase
            .from('answer_logs')
            .insert({
    ...
    async submitFeedback(logId, questionId, type, comment) {
    ...
    async submitManualAnswer(projectId, questionId, answerText, answerType, founderId) {
        // 1. Validation
        ...
        if (error)
            throw new Error(`Failed to save manual answer: ${error.message}`);
        ...
        await this.vectorStore.storeVectors(projectId, docGroupId, 1, [combinedText], [embedding], {
            source_type: 'MANUAL',
