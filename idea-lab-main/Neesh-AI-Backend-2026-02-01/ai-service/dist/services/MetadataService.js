...
class MetadataService {
     ...
     */
    extractMetadata(content, documentInfo, chunkIndex = 0) {
    ...
    buildMetadataFilter(criteria) {
        ...
        if (criteria.tags && criteria.tags.length > 0) {
            const tagConditions = criteria.tags.map(tag => `metadata->'tags' ? '${tag}'`).join(' OR ');
        ...
        if (criteria.topics && criteria.topics.length > 0) {
            const topicConditions = criteria.topics.map(topic => `metadata->'topics' ? '${topic}'`).join(' OR ');
     ...
     */
    assessExtractionQuality(content) {
     ...
     */
    calculateConfidenceScore(content, quality) {
     ...
     */
    generateTags(content, topics, entities) {
        const tags = new Set();
        // Add topics as tags
