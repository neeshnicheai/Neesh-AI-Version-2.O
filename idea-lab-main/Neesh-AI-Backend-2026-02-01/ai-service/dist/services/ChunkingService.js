...
class ChunkingService {
    constructor() {
        ...
        if (isNaN(this.chunkSize) || this.chunkSize < 50)
            this.chunkSize = 500;
        if (isNaN(this.chunkOverlap) || this.chunkOverlap < 0)
            this.chunkOverlap = 50;
        if (this.chunkOverlap >= this.chunkSize)
            this.chunkOverlap = Math.floor(this.chunkSize * 0.1);
        ...
        console.log(`[ChunkingService] Initialized — chunkSize: ${this.chunkSize}, ` +
            `chunkOverlap: ${this.chunkOverlap}`);
    }
     ...
     */
    async chunkTextWithSize(text, chunkSize, chunkOverlap) {
        if (!text)
            return [];
